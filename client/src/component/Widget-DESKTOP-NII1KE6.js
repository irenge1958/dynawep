...
console.log(arr[x].style)
  ...
  const removeondele = (pos) => {
    ...
    <div className="widget-container dark-theme">
      ...
      {[...modifications].reverse().map((x1,i)=>{
        ...
        const borderStyle = x1.type === "first" ? { border: '2px solid red', padding: '4px', borderRadius: '4px',cursor: 'pointer'  } : {cursor: 'pointer' };
        return(<>{styled(x1)==='grid-template-columns'||styled(x1)==='filter'||styled(x1).includes('transition')||styled(x1)==='box-shadow'?'':<div style={!(x1.suppressed == undefined || x1.suppressed === null)?{flexDirection:'column',color:'gray'}:{flexDirection:'column',color:'white'}}><div style={{display:"flex",alignItems: "center"}}><CodeIcon/><p style={!(x1.suppressed == undefined || x1.suppressed === null)?{flexDirection:'column',color:'gray'}:{color:"#4F46E5",}}>{x1.style}</p><CodeOffIcon/></div> <div style={!(x1.suppressed == undefined || x1.suppressed === null)?{display:'flex',color:'gray'}:{display:'flex',color:'white'}}>{!(x1.suppressed == undefined || x1.suppressed === null)?<button className="close-panel"><SettingsBackupRestoreIcon onClick={()=>removeondele(reversedIndex)} /></button>:<button className="close-panel" onClick={() => remove(reversedIndex)}>×</button>}<h2 style={{marginTop:'-5px',flex:1}}></h2><label style={{flex:9}} htmlFor="colorPicker"  >{x1.widget?x1.widget:`Change ${x1.element || x1.selector} ${styled(x1)}:`}</label>
 ...
 // Extract the value of the 2nd rule if it exists, otherwise the 1st
const value = (() => {
  const rules = x1.style
    .split(';')
    .map(s => s.trim())
    .filter(Boolean);

  const targetRule = rules[1] || rules[0]; // 👈 핵심 logic

  return targetRule?.split(':')[1]?.trim() || '';
})();
const hasPx  = haspx(value);   // "px"
const hasPer = hasper(value);  // "%"
const hasRem = hasrem(value);  // "rem"
const hasSec = hass(value);    // "s"
